...
def test(model, test_x, test_y, test_m, batch_size, unknowns):
    ...
    for i in range(test_n_batches):
        ...
        # eq = np.equal(preds, y)
        # eq = eq * m
        # acc = np.count_nonzero(eq) / m.sum()
        # print(actual[0][0][0])
        # print(y[0][:len(actual[0][0][0])])
        # print()
...
class Tagger(nn.Module):
    ...
    def __init__(self, vocab_size, embed_size, hidden_size, n_classes, embedding_vectors=None, train_embedding=True):
        ...
        self.crf = mycrf.CRF(len(tag2id)-2, 'cuda')
    ...
    def forward(self, x, mask,  y=None):
        ...
        if tags is not None:
            loss = self.crf.neg_log_likelihood_loss(x, mask, tags)
        else: 
            loss = 0
        scores, best_tag_sequence = self.crf.forward(x, mask)
        # class_probabilities = x * 0.0
        ...
        return x, {"loss": loss, "actual":best_tag_sequence} 
